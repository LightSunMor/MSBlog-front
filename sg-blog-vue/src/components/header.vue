...
<div class="">
	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(static/img/headbg05.jpg)'}">
		...
		<div class="h-information">
                    ...
                    <img :src="this.$store.state.themeObj.head_portrait?this.$store.state.themeObj.head_portrait:'static/img/Q万叶.jpg'" alt="">

			<h2 class="h-description">

                        {{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:"须臾时光 ， 驻晨曦旁"}}
...
export default {
	...
	mounted() { //页面元素加载完成,打字机效果
		var that = this;
		var timer = setTimeout(function() {
			Typeit(that.$store.state.themeObj.start_msg, "#luke");  //打字机效果
			clearTimeout(timer);
		}, 500);
	}
}
